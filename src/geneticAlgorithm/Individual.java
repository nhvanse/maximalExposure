...
//			
    ...
    public void makeMutation() { // change a random location by symmetrical one if can increase exposure)
        Random rand = new Random();
        int start = rand.nextInt(geneLength - 2) + 1;
        int end = start + rand.nextInt(geneLength - 1 - start);
        for (int i = start; i <= end; i++) {
            ...
            Location changedLoc = new Location((2 * medianLoc.x - currLoc.x), (2 * medianLoc.y - currLoc.y));

            if (changedLoc.x > net.wOfField || changedLoc.x < 0 || changedLoc.y > net.hOfField || changedLoc.y < 0) {
                continue;
            }
            if (net.exposureAt(changedLoc) >= net.exposureAt(currLoc)) path.set(i, changedLoc);
        }
        this.exposure = this.computeExposure();
    }
...
//		net.printInfo();
    ...
    public static void main(String[] args) throws Exception {
        ...
        saveToFile("./output/output2.txt", net, individ.path, individ.exposure);
